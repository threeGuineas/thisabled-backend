...
기능 축(명세 v2.3): MODE(시각·청각·발달·기본 UI) · SAFE(채팅 텍스트 위험 감지, 자체 모델) · COMM(LLM 소통 코치) · MATCH(SBERT+LightGBM 친구 추천) · CAPTION(Whisper 자막) · VISION(사진 설명).
...
## 명세 검증 (코드로 옮기기 전)
**SSOT는 `docs/ThisAbled_기능명세서_v2_3.md`** (식별자: ACC/TAG/FEED/POST/VISION/CAPTION/MATCH/FRIEND/BLOCK/CHAT/SAFE/COMM). 새 기능·엔드포인트·스키마는 착수 전 `brainstorming` 스킬로 의도·설계를 먼저 끌어내고, 해당 식별자 절을 읽어 모호함을 제거한다. 코드부터 뛰어들지 않는다.
명세 읽을 때 의심할 것: 수치 정합(한도·기간·길이) / 단정문 속 [TBD] / 식별자 중복 / 문서 간 동기화 / 시점 모순 / 코드–명세 드리프트.
발견한 결함은 **코드에서 임의 우회 금지** — 명세서를 고치거나 팀 안건으로 올린다.

## API·기능명세 변경 승인 게이트
- 다음 변경은 구현·문서 편집 전에 사용자에게 변경안을 제시하고 **명시적 승인을 받은 뒤** 착수한다.
  - API 경로·메서드, 요청·응답 필드, 필수/선택 여부, enum, 검증 한도, HTTP 상태·오류 계약
  - `docs/api.md`, `docs/ThisAbled_기능명세서_*.md`의 기능 계약 또는 문서 버전
- 승인 요청에는 `기존 → 제안 계약`, 변경 이유, 호환성(breaking 여부), 프론트엔드·DB·마이그레이션·OpenAPI·문서 버전 영향, 테스트·전환 계획을 포함한다.
- 사용자 승인 전에는 관련 코드와 명세 문서를 수정하지 않는다. 버그 수정 과정에서 계약 변경이 필요해진 경우도 작업을 멈추고 같은 절차를 적용한다.
- 승인 후 코드·테스트·OpenAPI·API 문서·SSOT 버전 및 변경 이력을 한 작업에서 동기화한다.
- PR 본문에 승인 범위와 승인된 대화 또는 안건의 날짜를 기록한다. 계약 변경이 없다면 그 사실을 명시한다.
...
## 링크
- 기능명세서(SSOT): `docs/ThisAbled_기능명세서_v2_3.md`
