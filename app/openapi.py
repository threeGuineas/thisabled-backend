...
OPERATION_GUIDES: dict[tuple[str, str], OperationGuide] = {
    ...
    ("get", "/api/v1/users/{user_id}"): _guide(
        "다른 사용자 프로필 조회",
        "공개 가능한 닉네임·자기소개·이미지·관심사와 활동 집계, 현재 친구 관계를 반환합니다. "
        "`stats.received_like_count`는 이 사용자의 공개 게시물이 받은 좋아요 수입니다. 생년월일, 미성년 "
        "여부, UI 모드는 노출하지 않습니다. 존재하지 않거나 어느 방향이든 차단 관계면 같은 404를 사용합니다.",
        "200과 공개 프로필을 반환합니다.",
        errors={404: "사용자가 없거나 차단 관계입니다. 두 경우를 화면에서 구분하지 마세요."},
    ),
    ("get", "/api/v1/users/{user_id}/posts"): _guide(
        "프로필 작성 게시물 조회",
        "프로필 사용자가 작성한 공개 게시물을 최신순 커서 페이지로 반환합니다. 내부 영상 드래프트는 제외하고, "
        "차단 관계에서는 사용자 존재를 숨기기 위해 404를 반환합니다. 다음 페이지는 `next_cursor`를 수정 없이 보내세요.",
        "200과 `items`, `next_cursor`를 반환합니다.",
        errors={400: "cursor 형식이 잘못되었습니다.", 404: "사용자가 없거나 차단 관계입니다."},
    ),
