...
## Corpus Check
- 88 files · ~34,441 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 941 nodes · 1771 edges · 73 communities (52 shown, 21 thin omitted)
...
Cohesion: 0.06
Nodes (41): authorize(), callback(), logout(), AsyncSession, ACC-01/02 — 소셜 OAuth 전용 인증.  흐름: authorize → 제공자 로그인 → callback   - 기가입자: 즉시 로그인, refresh(), _set_refresh_cookie(), signup() (+33 more)

### Community 3 - "Posts API & DB Models"
Cohesion: 0.07
Nodes (47): do_run_migrations(), run_async_migrations(), run_migrations_online(), get_current_user(), AsyncSession, Base, get_db(), ChatMessage (+39 more)
...
Cohesion: 0.16
Nodes (18): create_block(), list_blocks(), AsyncSession, User, UUID, BLOCK-01 사용자 차단 — 게시물·프로필·요청·채팅·추천 상호 제거., remove_block(), BlockIn (+10 more)
...
Cohesion: 0.27
Nodes (19): accept_request(), _author(), cancel_request(), decline_request(), _get_pending(), list_friends(), list_requests(), AsyncSession (+11 more)
...
Cohesion: 0.07
Nodes (56): ChatRoom, 1:1 채팅방 (CHAT-01/02). request=요청함, active=일반 채팅.      참여자는 정규화 쌍(user_a < user_b, ChatPolicyError, counterpart_id(), flagged_count_since_reset(), get_or_create_room(), has_active_restriction(), AsyncSession (+48 more)
...
Cohesion: 0.07
Nodes (25): Redis, WS /api/v1/ws?token=<access> — 새 메시지·알림 실시간 푸시.  인증은 JWT 검증만으로 처리(DB 미조회)해 연결 비용, _relay(), ws_endpoint(), Settings, get_redis(), get_redis_client(), Redis (+17 more)
...
Cohesion: 0.12
Nodes (36): accept_request(), _author(), create_room(), _get_my_room(), list_messages(), list_requests(), list_rooms(), _message_out() (+28 more)
...
Cohesion: 0.06
Nodes (52): complete(), hints(), HintsOut, AsyncSession, User, COMM-01~05 — AI 소통 코치. 전 모드 제공, 발달 모드 기본 노출(§14)., COMM-05: 참여자 검증 후 최근 N개 텍스트. flagged 미열람·pending은 제외., COMM-02: 제안일 뿐, 자동 입력·게시하지 않는다 (§20-8). (+44 more)
...
- **What connects `match-model mock — SBERT+LightGBM 모델 서버가 오기 전 자리 지킴이.  계약: POST /score {me, cand`, `safety-model mock — AI 팀원의 자체 파인튜닝 안전 모델 서버가 오기 전 자리 지킴이.  계약: POST /analyze {te`, `COMM-01~05 — AI 소통 코치. 전 모드 제공, 발달 모드 기본 노출(§14).` to the rest of the system?**
