...
- [[_COMMUNITY_get_describe_caller|get_describe_caller]]
...
Cohesion: 0.06
Nodes (82): accept_request(), _author(), create_room(), _get_my_room(), list_messages(), list_requests(), list_rooms(), _message_out() (+74 more)
...
Cohesion: 0.11
Nodes (52): _author_out(), _blocked_ids_subq(), caption_status(), _comment_out(), create_comment(), create_post(), delete_comment(), delete_post() (+44 more)

### Community 3 - "AI Media & Quota"
Cohesion: 0.18
Nodes (22): _cache_get(), _cache_put(), caption_chat_message_job(), caption_post_media_job(), describe_chat_message_job(), describe_image(), describe_post_media_job(), file_path_from_url() (+14 more)

### Community 5 - "Friends & Blocks Domain"
Cohesion: 0.06
Nodes (70): do_run_migrations(), run_async_migrations(), run_migrations_online(), create_block(), list_blocks(), AsyncSession, UUID, BLOCK-01 사용자 차단 — 게시물·프로필·요청·채팅·추천 상호 제거. (+62 more)

### Community 6 - "Media Endpoints & DB Session"
Cohesion: 0.11
Nodes (26): async_sessionmaker, AsyncSession, BackgroundTasks, Redis, UploadFile, 미디어 업로드 — 사진(≤3장) · 영상(드래프트+자막 시작) · VIS-03 음성 입력., VIS-03 음성 입력 — 결과는 입력란 삽입용. 자동 게시하지 않는다 (§20-4)., 사진 업로드 (post 미연결) — POST /posts 의 media_ids로 연결한다. (+18 more)

### Community 7 - "Test Helpers & Fixtures"
Cohesion: 0.05
Nodes (93): user_channel(), get_safety_client(), FastAPI 의존성 — 테스트에서 fake로 override., auth_header(), client(), _force_oauth_mock(), AsyncClient, 테스트 공용 픽스처.  각 테스트는 단일 커넥션 위의 외부 트랜잭션에서 실행되고 끝나면 롤백된다. 앱 코드가 `await db.commit()` (+85 more)
...
Cohesion: 0.06
Nodes (38): complete(), hints(), HintsOut, AsyncSession, UUID, COMM-01~05 — AI 소통 코치. 전 모드 제공, 발달 모드 기본 노출(§14)., COMM-05: 참여자 검증 후 최근 N개 텍스트. flagged 미열람·pending은 제외., COMM-02: 제안일 뿐, 자동 입력·게시하지 않는다 (§20-8). (+30 more)
...
Cohesion: 0.14
Nodes (25): AsyncSession, RecommendationListOut, RecommendationOut, recommendations(), age_band(), full_age(), is_minor(), 만 나이 계산 (§4.5 미성년 보호, MATCH-02-5 연령대).  생년월일 원문은 저장만 하고 파생값(만 나이·연령대·미성년)은 항상 요청 (+17 more)

### Community 13 - "Auth Endpoints & Signup"
Cohesion: 0.08
Nodes (15): authorize(), get_provider(), GoogleProvider, _http(), KakaoProvider, MockProvider, OAuthProvider, OAuthUserInfo (+7 more)

### Community 14 - "App Config & Bootstrap"
Cohesion: 0.24
Nodes (16): Any, Path, test_fixture_contains_detection_and_boundary_cases(), test_fixture_rows_match_case_schema(), test_invalid_case_is_rejected(), test_invalid_verdict_is_error_not_safe(), test_summarize_reports_recall_fnr_fpr_and_subgroups(), evaluate() (+8 more)
...
Cohesion: 0.08
Nodes (38): callback(), _frontend_redirect(), logout(), AsyncSession, UUID, ACC-01/02 — 소셜 OAuth 전용 인증.  흐름: authorize → 제공자 로그인 → callback → FRONTEND_URL로, 콜백 결과를 프론트 SPA로 302 전달 — 브라우저가 직접 호출하므로 JSON 대신 리다이렉트., refresh() (+30 more)
...
Cohesion: 0.16
Nodes (19): consume_vision(), 일 20회·분 5회 동시 검증 — 분 한도 초과 시 일 카운터 롤백., caption_key(), _minute(), Redis, Redis TTL 카운터 — VISION-01·CAPTION-01 사용자별 호출 한도.  키는 날짜/분 단위로 자연 만료(TTL)되고, 자막 실, 카운터 1 증가. 한도 초과면 롤백 후 False., CAPTION-01: 자막 생성 실패(재시도 소진) 시 일일 횟수 차감 복원. (+11 more)
...
Cohesion: 0.18
Nodes (14): PostMedia, 게시물 미디어 (사진≤3·영상 1개는 앱 검증). VISION 설명·CAPTION 자막 부착., cleanup_stale_drafts(), APScheduler 인프로세스 크론 — 24h 드래프트 청소 (+ Task 11에서 미분석 재분석 추가)., CAPTION-01: 업로드 후 24시간 미게시 내부 드래프트를 영상·자막과 함께 삭제.      이미 성공한 자막 생성의 일일 횟수 차감은 복, CallCounter, fake_ai(), VISION-01 사진 설명 · CAPTION-01 영상 자막 · VIS-03 음성 입력 · 24h 드래프트 청소. (+6 more)
...
Cohesion: 0.16
Nodes (10): Settings, generate_description(), _get_client(), AsyncOpenAI, GPT-4o Vision 이미지 해설 서비스 (F02_S04 시각장애 모드).  엔드포인트는 `vision.generate_description, 이미지 바이트 → 한국어 해설 텍스트. (실제 GPT-4o 호출), BaseSettings, v2.1 명세 고정값이 Settings에 반영되어 있는지 검증. (+2 more)
...
## Knowledge Gaps
- **250 isolated node(s):** `실행 기준`, `결과`, `결론`, `FN`, `FP` (+245 more)
...
- **Why does `User` connect `Posts & Media Domain` to `Chat & SAFE Domain`, `User Models & Schemas`, `Friends & Blocks Domain`, `Media Endpoints & DB Session`, `COMM Coach & STT Clients`, `OAuth Providers`, `Auth Tests`, `Real OAuth Tests`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `auth_header()` connect `Test Helpers & Fixtures` to `test_media.py`, `Auth Tests`, `quota.py`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `register()` connect `Test Helpers & Fixtures` to `test_media.py`, `Auth Tests`, `quota.py`?**
...
- **What connects `실행 기준`, `결과`, `결론` to the rest of the system?**
