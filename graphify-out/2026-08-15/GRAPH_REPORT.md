...
## Corpus Check
- 135 files · ~67,206 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1333 nodes · 3370 edges · 145 communities (100 shown, 45 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 293 edges (avg confidence: 0.6)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `edc326d5`
...
## Communities (145 total, 45 thin omitted)

### Community 0 - "users.py"
Cohesion: 0.09
Nodes (51): me(), _me_out(), patch_me(), patch_settings(), public_profile(), put_mode(), put_tags(), AsyncSession (+43 more)

### Community 1 - "StrictRequest"
Cohesion: 0.16
Nodes (27): PostCategory, POST-01 — Figma의 게시물 단일 카테고리 5종., BaseModel, API 요청 DTO의 공통 검증 정책., 프론트 필드 오타를 조용히 무시하지 않고 422로 드러낸다., StrictRequest, CaptionStatusOut, ImageUploadOut (+19 more)

### Community 2 - "User"
Cohesion: 0.07
Nodes (81): _author_out(), _blocked_ids_subq(), caption_status(), _comment_out(), _contains_pattern(), create_comment(), create_post(), _cursor_position() (+73 more)

### Community 3 - "ai_media.py"
Cohesion: 0.12
Nodes (30): content_type_from_path(), STT multipart에 파일 확장자와 일치하는 실제 MIME을 제공한다., _acquire_caption_lock(), caption_chat_message_job(), caption_post_media_job(), describe_chat_message_job(), describe_post_media_job(), file_path_from_url() (+22 more)

### Community 5 - "friends.py"
Cohesion: 0.16
Nodes (29): accept_request(), _author(), cancel_request(), decline_request(), _get_pending(), list_friends(), list_requests(), AsyncSession (+21 more)

### Community 6 - "upload_video"
Cohesion: 0.13
Nodes (21): async_sessionmaker, AsyncSession, BackgroundTasks, UploadFile, VIS-03 음성 입력 — 결과는 입력란 삽입용. 자동 게시하지 않는다 (§20-4)., 사진 업로드 (post 미연결) — POST /posts 의 media_ids로 연결한다., 영상 첨부 = processing 내부 드래프트 생성 + 자막 생성 즉시 시작 (POST-01·CAPTION-01). FE 길이는 빠른 거부용…, transcribe_voice() (+13 more)

### Community 7 - "health.py"
Cohesion: 0.40
Nodes (4): health(), HealthOut, BaseModel, get

### Community 8 - "CallSignalType"
Cohesion: 0.29
Nodes (17): CallKind, CallSignalType, CallCreateIn, CallOut, CallSignalIn, CallSignalOut, MessageIn, MessageListOut (+9 more)

### Community 9 - "register"
Cohesion: 0.12
Nodes (42): auth_header(), mock OAuth 가입 헬퍼 → {access_token, user_id, ...}., register(), VISION-01 사진 설명 · CAPTION-01 영상 자막 · VIS-03 음성 입력 · 24h 드래프트 청소., 새 Starlette에서는 응답 본문 뒤 background task가 별도 스케줄될 수 있어 완료를 기다린다., test_caption_failure_requires_explicit_choice_and_refunds(), test_cleanup_deletes_stale_drafts(), test_failed_caption_can_be_retried() (+34 more)
...
Cohesion: 0.19
Nodes (18): logout(), AsyncSession, UUID, ACC-01/02 — 소셜 OAuth 전용 인증. 흐름: authorize → 제공자 로그인 → callback → FRONTEND_URL로…, refresh(), _set_refresh_cookie(), signup(), decode_signup_token() (+10 more)
...
### Community 17 - "v1/media.py"
Cohesion: 0.18
Nodes (16): 미디어 업로드 — 사진(≤3장) · 영상(드래프트+자막 시작) · VIS-03 음성 입력., list_notifications(), mark_read(), NotificationListOut, NotificationOut, AsyncSession, BaseModel, get (+8 more)

### Community 18 - "match.py"
Cohesion: 0.18
Nodes (16): AsyncSession, BaseModel, get, RecommendationListOut, RecommendationOut, recommendations(), get_match_client(), MatchClient (+8 more)
...
### Community 21 - "test_config.py"
Cohesion: 0.18
Nodes (10): Settings, BaseSettings, 테스트 전용 DB만 허용한다. 명시값이 없으면 현재 DB명에 `_test`를 붙인다., _test_database_url(), v2.1 명세 고정값이 Settings에 반영되어 있는지 검증., docker compose env_file은 POSTGRES_PASSWORD/REDIS_PASSWORD도 컨테이너 프로세스에 노출한다. 이…, CI처럼 Docker 볼륨이 없는 환경에서도 테스트 수집 전 app import가 성공해야 한다., test_app_import_does_not_require_existing_upload_directory() (+2 more)
...
### Community 30 - "blocks.py"
Cohesion: 0.19
Nodes (17): BlockedOut, create_block(), list_blocks(), AsyncSession, BaseModel, delete, get, UUID (+9 more)
...
Cohesion: 0.21
Nodes (10): close_redis_client(), 현재 워커 이벤트 루프의 연결 풀을 애플리케이션 종료 시 정리한다., add_security_headers(), lifespan(), FastAPI, stop_scheduler(), middleware, Request (+2 more)
...
Cohesion: 0.06
Nodes (95): accept_request(), _author(), create_room(), get_call(), _get_my_room(), _latest_messages(), list_messages(), list_requests() (+87 more)
...
### Community 48 - "ws.py"
Cohesion: 0.29
Nodes (8): WS /api/v1/ws?token=<access> — 새 메시지·알림·통화 시그널 실시간 푸시. 인증은 JWT 검증만으로 처리(DB…, _relay(), ws_endpoint(), 실시간 이벤트 — Redis pub/sub 팬아웃 (다중 워커에서도 사용자별 채널로 전달). event 형식: {"type":…, user_channel(), 운영 Redis(db 0)에 직접 publish — pub/sub은 잔존 데이터가 없어 안전., test_ws_relays_published_event(), WebSocket
...
Cohesion: 0.14
Nodes (35): test_call_requires_online_active_same_age_friend(), _describe_fake(), make_friends(), CHAT-01 친구 채팅 · CHAT-02 비친구 요청 · 미디어 제한 (§4.5)., _room(), _send(), test_chat_text_is_trimmed_and_limited(), test_chat_video_gets_caption_and_notifies_both_users() (+27 more)
...
Cohesion: 0.29
Nodes (6): generate_description(), _get_client(), AsyncOpenAI, GPT-4o Vision 이미지 해설 서비스 (F02_S04 시각장애 모드). 엔드포인트는…, 이미지 바이트 → 한국어 해설 텍스트. (실제 GPT-4o 호출), RuntimeError

### Community 83 - "timedelta"
Cohesion: 0.46
...
- `caption_post_media_job()` (3× useful, score=2.996127272)
- `caption_chat_message_job()` (3× useful, score=2.436997324)
- `upload_video()` (2× useful, score=1.997813548)
- `publish_post()` (2× useful, score=1.997813548)
- `save_upload()` (2× useful, score=1.996465752)
- `cleanup_stale_drafts()` (2× useful, score=1.996465752)
- `transcribe_segments()` (2× useful, score=1.996465752)
- `UiMode` (2× useful, score=1.995526653)
- `user_features()` (2× useful, score=1.995526653)

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `User` connect `User` to `callback`, `users.py`, `friends.py`, `upload_video`, `v1/chat.py`, `v1/comm.py`, `v1/auth.py`, `v1/media.py`, `match.py`, `blocks.py`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `_Redis` connect `_Redis` to `callback`, `try_consume`, `User`, `ai_media.py`, `generate_caption`, `friends.py`, `upload_video`, `v1/chat.py`, `quota.py`, `ws.py`, `call_signaling.py`, `test_ws.py`, `scheduler.py`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `StrictRequest` connect `StrictRequest` to `users.py`, `User`, `friends.py`, `CallSignalType`, `v1/comm.py`, `v1/auth.py`, `v1/media.py`, `blocks.py`?**
