...
## Summary
- 1238 nodes · 3043 edges · 126 communities (81 shown, 45 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 200 edges (avg confidence: 0.65)
...
Cohesion: 0.06
Nodes (72): me(), _me_out(), patch_me(), patch_settings(), public_profile(), put_mode(), put_tags(), AsyncSession (+64 more)

### Community 2 - "User"
Cohesion: 0.09
Nodes (69): _author_out(), _blocked_ids_subq(), caption_status(), _comment_out(), _contains_pattern(), create_comment(), create_post(), _cursor_position() (+61 more)

### Community 3 - "ai_media.py"
Cohesion: 0.12
Nodes (34): content_type_from_path(), STT multipart에 파일 확장자와 일치하는 실제 MIME을 제공한다., _acquire_caption_lock(), _cache_get(), _cache_put(), caption_chat_message_job(), caption_post_media_job(), describe_chat_message_job() (+26 more)

### Community 5 - "friends.py"
Cohesion: 0.19
Nodes (26): accept_request(), _author(), cancel_request(), decline_request(), _get_pending(), list_friends(), list_requests(), AsyncSession (+18 more)

### Community 6 - "blocks.py"
Cohesion: 0.16
Nodes (20): BlockedOut, create_block(), list_blocks(), AsyncSession, BaseModel, delete, get, UUID (+12 more)

### Community 7 - "register"
Cohesion: 0.16
Nodes (31): auth_header(), mock OAuth 가입 헬퍼 → {access_token, user_id, ...}., register(), test_caption_failure_requires_explicit_choice_and_refunds(), test_failed_caption_can_be_retried(), test_global_caption_limit_blocks_upload(), test_recovery_restarts_processing_caption(), test_transcribe_voice_input() (+23 more)
...
Cohesion: 0.14
Nodes (35): BaseModel, RestrictionReleaseOut, _coach(), complete(), hints(), HintsOut, _post_context(), PostIn (+27 more)

### Community 13 - "v1/auth.py"
Cohesion: 0.05
Nodes (58): authorize(), callback(), _frontend_redirect(), logout(), AsyncSession, get, UUID, ACC-01/02 — 소셜 OAuth 전용 인증. 흐름: authorize → 제공자 로그인 → callback → FRONTEND_URL로… (+50 more)
...
Cohesion: 0.13
Nodes (16): Settings, get_redis(), get_redis_client(), 현재 이벤트 루프 전용 클라이언트. asyncio Redis 연결을 다른 루프에서 재사용하면 Future/transport가 깨지므로…, FastAPI 의존성. 테스트에서 override 가능하도록 분리., BaseSettings, _conn(), 테스트 공용 픽스처. 각 테스트는 단일 커넥션 위의 외부 트랜잭션에서 실행되고 끝나면 롤백된다. 앱 코드가 `await db.commit()`… (+8 more)
...
Cohesion: 0.17
Nodes (17): _add_error_response(), _guide(), install_openapi(), OperationGuide, Any, FastAPI, 프론트엔드 구현자를 위한 OpenAPI 문서 보강. FastAPI의 기본 스키마는 타입은 정확하지만 비즈니스 상태와 화면 처리 방법을 설명하지…, 프론트엔드 계약 문서가 라우트 변경 뒤에도 빠지거나 퇴행하지 않도록 검증. (+9 more)
...
Cohesion: 0.06
Nodes (89): accept_request(), _author(), create_room(), _get_my_room(), _latest_messages(), list_messages(), list_requests(), list_rooms() (+81 more)
...
Cohesion: 0.19
Nodes (20): _describe_fake(), make_friends(), CHAT-01 친구 채팅 · CHAT-02 비친구 요청 · 미디어 제한 (§4.5)., _room(), test_chat_video_gets_caption_and_notifies_both_users(), test_friend_chat_roundtrip(), test_media_blocked_in_request_room(), test_media_only_between_adult_friends() (+12 more)
...
Cohesion: 0.16
Nodes (21): age_band(), full_age(), is_minor(), 만 나이 계산 (§4.5 미성년 보호, MATCH-02-5 연령대). 생년월일 원문은 저장만 하고 파생값(만 나이·연령대·미성년)은 항상 요청…, 만 14~18세 미성년 보호 대상 여부 (§4.5). 만 13 이하는 가입 자체가 불가., MATCH-02-5 연령대. 추천 특성으로만 사용, 생년월일 원문은 모델에 전달하지 않는다., normalize_pair(), UUID (+13 more)

### Community 66 - "test_media.py"
Cohesion: 0.12
Nodes (23): PostMedia, 게시물 미디어 (사진≤3·영상 1개는 앱 검증). VISION 설명·CAPTION 자막 부착., cleanup_stale_drafts(), APScheduler 크론 — 드래프트 청소·자막 작업 복구·SAFE 재분석., CAPTION-01: 업로드 후 24시간 미게시 내부 드래프트를 영상·자막과 함께 삭제. 이미 성공한 자막 생성의 일일 횟수 차감은 복원하지…, §18.3: 모델 복구 감지용 주기 재분석 (unanalyzed·pending 텍스트)., _reanalyze_job(), start_scheduler() (+15 more)
...
### Community 89 - "_BorrowedSession"
Cohesion: 0.29
Nodes (4): _BorrowedSession, client(), AsyncClient, 백그라운드 잡이 테스트 공유 세션을 닫지 않도록 하는 async context manager.

### Community 108 - "test_ws.py"
Cohesion: 0.14
Nodes (13): 실시간 이벤트 — Redis pub/sub 팬아웃 (다중 워커에서도 사용자별 채널로 전달). event 형식: {"type":…, user_channel(), FakeSafety, fixture, 돈' 포함 → flagged. fail=True → SafetyUnavailable (§18.3)., safety(), fixture, WS 푸시 — 토큰 검증, pub/sub 릴레이, 채팅 전송 이벤트. (+5 more)
...
### Community 117 - "v1/media.py"
Cohesion: 0.11
Nodes (25): health(), HealthOut, BaseModel, get, 미디어 업로드 — 사진(≤3장) · 영상(드래프트+자막 시작) · VIS-03 음성 입력., list_notifications(), mark_read(), NotificationListOut (+17 more)
...
- `caption_chat_message_job()` (3× useful, score=2.438754491)
- `save_upload()` (2× useful, score=1.997905279)
- `caption_post_media_job()` (2× useful, score=1.997905279)
- `caption_key()` (2× useful, score=1.997905279)
- `cleanup_stale_drafts()` (2× useful, score=1.997905279)
- `transcribe_segments()` (2× useful, score=1.997905279)
- `recommendations()` (2× useful, score=1.996965503)
- `UiMode` (2× useful, score=1.996965503)
- `user_features()` (2× useful, score=1.996965503)

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `User` connect `User` to `match.py`, `users.py`, `test_withdrawal.py`, `upload_video`, `friends.py`, `blocks.py`, `v1/chat.py`, `v1/comm.py`, `v1/auth.py`, `v1/media.py`?**
  _High betweenness centrality (0.071) - this node is a cross-community bridge._
- **Why does `auth_header()` connect `register` to `test_withdrawal.py`, `test_media.py`, `test_safe.py`, `test_match.py`, `test_ws.py`, `test_auth.py`, `conftest.py`, `test_friends.py`, `test_chat.py`, `test_notifications.py`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `register()` connect `register` to `test_withdrawal.py`, `test_media.py`, `test_safe.py`, `test_match.py`, `test_ws.py`, `test_auth.py`, `conftest.py`, `test_friends.py`, `test_chat.py`, `test_notifications.py`, `_BorrowedSession`?**
...
- **Should `User` be split into smaller, more focused modules?**
  _Cohesion score 0.09246575342465753 - nodes in this community are weakly interconnected._