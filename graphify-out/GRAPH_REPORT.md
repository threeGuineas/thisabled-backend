...
## Summary
- 1286 nodes · 3245 edges · 137 communities (92 shown, 45 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 232 edges (avg confidence: 0.63)
...
### Community 2 - "User"
Cohesion: 0.05
Nodes (100): async_sessionmaker, AsyncSession, BackgroundTasks, UploadFile, 미디어 업로드 — 사진(≤3장) · 영상(드래프트+자막 시작) · VIS-03 음성 입력., VIS-03 음성 입력 — 결과는 입력란 삽입용. 자동 게시하지 않는다 (§20-4)., 사진 업로드 (post 미연결) — POST /posts 의 media_ids로 연결한다., 영상 첨부 = processing 내부 드래프트 생성 + 자막 생성 즉시 시작 (POST-01·CAPTION-01). FE 길이는 빠른 거부용… (+92 more)
...
### Community 6 - "Block"
Cohesion: 0.14
Nodes (19): BlockedOut, create_block(), list_blocks(), AsyncSession, BaseModel, delete, get, UUID (+11 more)

### Community 7 - "register"
Cohesion: 0.12
Nodes (40): auth_header(), mock OAuth 가입 헬퍼 → {access_token, user_id, ...}., register(), test_post_comments_hide_blocked_post(), test_post_comments_use_post_and_latest_10_comments(), test_simplify_and_complete(), MATCH — 후보 제외 규칙은 백엔드 강제, 점수·사유는 모델 서버 (mock/fake)., 도메인 default는 MATCH 서버가 허용하는 중립값으로 직렬화한다. (+32 more)

### Community 8 - "_Redis"
Cohesion: 0.20
Nodes (26): accept_request(), create_room(), get_call(), _get_my_room(), _latest_messages(), list_messages(), list_requests(), list_rooms() (+18 more)

### Community 9 - "FakeMatch"
Cohesion: 0.40
...
Cohesion: 0.19
Nodes (23): authorize(), callback(), logout(), AsyncSession, get, UUID, ACC-01/02 — 소셜 OAuth 전용 인증. 흐름: authorize → 제공자 로그인 → callback → FRONTEND_URL로…, refresh() (+15 more)
...
Cohesion: 0.18
Nodes (12): _BorrowedSession, client(), _force_oauth_mock(), AsyncClient, fixture, 테스트 공용 픽스처. 각 테스트는 단일 커넥션 위의 외부 트랜잭션에서 실행되고 끝나면 롤백된다. 앱 코드가 `await db.commit()`…, .env의 실키·OAUTH_MOCK=false는 배포용 — 테스트는 항상 mock 제공자로 결정론적으로 실행한다. 개별…, 백그라운드 잡이 테스트 공유 세션을 닫지 않도록 하는 async context manager. (+4 more)
...
### Community 30 - "Base"
Cohesion: 0.14
Nodes (20): Base, AiResultCache, Notification, 알림 (§16). WS 푸시와 병행 저장., 동일 미디어 해시 캐싱 (VISION-01·CAPTION-01 중복 호출 방지, 비용 방어)., Friendship, 양방향 친구 관계 (FRIEND-02). 정규화 쌍: user_a < user_b., ForbiddenNickname (+12 more)
...
Cohesion: 0.10
Nodes (27): health(), HealthOut, BaseModel, get, lifespan(), FastAPI, _add_error_response(), _guide() (+19 more)
...
Cohesion: 0.12
Nodes (38): _room_out(), §CHAT-02 — request=요청함, active=일반 채팅., RoomState, ChatMessage, ChatReadState, ChatRoom, datetime, SAFE-05 관계 단위 자동 전송 제한. 누적 카운터는 별도 컬럼 없이 chat_messages에서 `flagged AND… (+30 more)
...
### Community 55 - "stt.py"
Cohesion: 0.06
Nodes (32): Settings, _extract_audio_to_m4a(), _get_client(), AsyncOpenAI, Path, OpenAI Whisper STT 서비스 (F02_S05 음성 댓글 / 자막). 엔드포인트는 `stt.transcribe(...)` 를…, 오디오 바이트 → 한국어 전사 텍스트. (실제 Whisper 호출, VIS-03 음성 입력), 최대 200MB 영상을 STT 상한 안의 단일 채널 M4A 음성으로 정규화한다. (+24 more)
...
### Community 66 - "v1/chat.py"
Cohesion: 0.26
Nodes (21): _author(), _message_out(), _message_preview(), CHAT-01~05 — 1:1 채팅 · 요청함 · 읽음 · WebRTC 시그널링., CallKind, CallSignalType, CallCreateIn, CallOut (+13 more)
...
### Community 89 - "test_ws.py"
Cohesion: 0.15
Nodes (25): WS /api/v1/ws?token=<access> — 새 메시지·알림·통화 시그널 실시간 푸시. 인증은 JWT 검증만으로 처리(DB…, _relay(), ws_endpoint(), user_channel(), heartbeat(), mark_offline(), mark_online(), presence_key() (+17 more)

### Community 90 - "call_signaling.py"
Cohesion: 0.27
...
### Community 121 - "storage.py"
Cohesion: 0.24
Nodes (11): delete_upload(), _ext_from_content_type(), Path, Protocol, Save file locally and return the public URL path., 업로드를 메모리에 통째로 올리지 않고 저장하며 크기와 SHA-256을 계산한다., save_upload(), save_upload_stream() (+3 more)
...
Cohesion: 0.39
Nodes (7): AccessTokenOut, Agreements, AuthorizeOut, BaseModel, 가입 필수 동의 3종 (ACC-01, §17 외부 AI 데이터 처리 안내 포함)., SignupIn, TokenOut
...
### Community 133 - "SafetyUnavailable"
Cohesion: 0.22
...
Cohesion: 0.38
Nodes (5): 실시간 이벤트 — Redis pub/sub 팬아웃 (다중 워커에서도 사용자별 채널로 전달). event 형식: {"type":…, _enabled(), notify(), AsyncSession, §16 알림 생성 — DB 저장 + WS 푸시 병행. 표현(시각·음성·진동)은 FE가 모드별 처리.

### Community 136 - "_frontend_redirect"
Cohesion: 0.67
...
- `caption_chat_message_job()` (3× useful, score=2.438494427)
- `save_upload()` (2× useful, score=1.997692227)
- `caption_post_media_job()` (2× useful, score=1.997692227)
- `caption_key()` (2× useful, score=1.997692227)
- `cleanup_stale_drafts()` (2× useful, score=1.997692227)
- `transcribe_segments()` (2× useful, score=1.997692227)
- `recommendations()` (2× useful, score=1.996752551)
- `UiMode` (2× useful, score=1.996752551)
- `user_features()` (2× useful, score=1.996752551)

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `User` connect `User` to `match.py`, `users.py`, `v1/chat.py`, `friends.py`, `Block`, `_Redis`, `services/chat.py`, `v1/comm.py`, `v1/auth.py`, `test_auth.py`, `send_media`, `session.py`, `Base`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `_Redis` connect `_Redis` to `User`, `ai_media.py`, `generate_caption`, `friends.py`, `notify`, `quota.py`, `send_media`, `stt.py`, `test_ws.py`, `call_signaling.py`?**
  _High betweenness centrality (0.026) - this node is a cross-community bridge._
- **Why does `_post()` connect `register` to `User`, `friends.py`, `Block`, `_Redis`, `match_model.py`, `v1/comm.py`, `v1/auth.py`, `send_media`, `session.py`, `safety_model.py`?**
