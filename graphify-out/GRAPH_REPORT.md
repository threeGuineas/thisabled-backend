...
## Summary
- 1230 nodes · 3025 edges · 122 communities (77 shown, 45 thin omitted)
...
Cohesion: 0.09
Nodes (34): AsyncSession, BaseModel, get, RecommendationListOut, RecommendationOut, recommendations(), age_band(), full_age() (+26 more)

### Community 1 - "users.py"
Cohesion: 0.11
Nodes (45): me(), _me_out(), patch_me(), patch_settings(), public_profile(), put_mode(), put_tags(), AsyncSession (+37 more)

### Community 2 - "User"
Cohesion: 0.06
Nodes (93): async_sessionmaker, AsyncSession, BackgroundTasks, UploadFile, VIS-03 음성 입력 — 결과는 입력란 삽입용. 자동 게시하지 않는다 (§20-4)., 사진 업로드 (post 미연결) — POST /posts 의 media_ids로 연결한다., 영상 첨부 = processing 내부 드래프트 생성 + 자막 생성 즉시 시작 (POST-01·CAPTION-01). FE 길이는 빠른 거부용…, transcribe_voice() (+85 more)
...
Cohesion: 0.09
Nodes (48): BlockedOut, create_block(), list_blocks(), AsyncSession, BaseModel, delete, get, UUID (+40 more)
...
Cohesion: 0.12
Nodes (42): auth_header(), mock OAuth 가입 헬퍼 → {access_token, user_id, ...}., register(), test_post_comments_use_post_and_latest_10_comments(), test_simplify_and_complete(), VISION-01 사진 설명 · CAPTION-01 영상 자막 · VIS-03 음성 입력 · 24h 드래프트 청소., 새 Starlette에서는 응답 본문 뒤 background task가 별도 스케줄될 수 있어 완료를 기다린다., test_caption_failure_requires_explicit_choice_and_refunds() (+34 more)
...
Cohesion: 0.13
Nodes (36): _coach(), complete(), hints(), HintsOut, _post_context(), PostIn, AsyncSession, BaseModel (+28 more)

### Community 13 - "v1/auth.py"
Cohesion: 0.05
Nodes (49): authorize(), callback(), _frontend_redirect(), logout(), AsyncSession, get, UUID, ACC-01/02 — 소셜 OAuth 전용 인증. 흐름: authorize → 제공자 로그인 → callback → FRONTEND_URL로… (+41 more)
...
### Community 33 - "ws.py"
Cohesion: 0.20
Nodes (18): WS /api/v1/ws?token=<access> — 새 메시지·알림 실시간 푸시. 인증은 JWT 검증만으로 처리(DB 미조회)해 연결…, _relay(), ws_endpoint(), get_redis(), get_redis_client(), 현재 이벤트 루프 전용 클라이언트. asyncio Redis 연결을 다른 루프에서 재사용하면 Future/transport가 깨지므로…, FastAPI 의존성. 테스트에서 override 가능하도록 분리., heartbeat() (+10 more)
...
Cohesion: 0.14
Nodes (21): lifespan(), FastAPI, _add_error_response(), _guide(), install_openapi(), OperationGuide, Any, FastAPI (+13 more)
...
### Community 117 - "Base"
Cohesion: 0.07
Nodes (39): do_run_migrations(), run_async_migrations(), run_migrations_online(), list_notifications(), mark_read(), NotificationListOut, NotificationOut, AsyncSession (+31 more)
...
Cohesion: 0.12
Nodes (22): 미디어 업로드 — 사진(≤3장) · 영상(드래프트+자막 시작) · VIS-03 음성 입력., delete_upload(), _ext_from_content_type(), Exception, Path, Protocol, Save file locally and return the public URL path., 업로드를 메모리에 통째로 올리지 않고 저장하며 크기와 SHA-256을 계산한다. (+14 more)
...
- `caption_chat_message_job()` (3× useful, score=2.438909583)
- `save_upload()` (2× useful, score=1.998032336)
- `caption_post_media_job()` (2× useful, score=1.998032336)
- `caption_key()` (2× useful, score=1.998032336)
- `cleanup_stale_drafts()` (2× useful, score=1.998032336)
- `transcribe_segments()` (2× useful, score=1.998032336)
- `recommendations()` (2× useful, score=1.9970925)
- `UiMode` (2× useful, score=1.9970925)
- `user_features()` (2× useful, score=1.9970925)

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `User` connect `User` to `match.py`, `users.py`, `friends.py`, `v1/chat.py`, `v1/comm.py`, `v1/auth.py`, `Base`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Why does `auth_header()` connect `register` to `test_ws.py`, `test_auth.py`, `Block`, `test_friends.py`, `Base`, `test_chat.py`, `test_notifications.py`, `conftest.py`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Why does `register()` connect `register` to `test_ws.py`, `test_auth.py`, `Block`, `test_friends.py`, `Base`, `test_chat.py`, `test_notifications.py`, `conftest.py`?**
